...
int perform_dma_write(uint32_t dest_addr, uint32_t* payload, size_t len) {
    ...
    // Allocate physically contiguous memory for control block
    // In real implementation, would use DMA-capable memory
    dma_cb_t* cb = aligned_alloc(32, sizeof(dma_cb_t));
    if (!cb) {
    ...
    uint32_t* src_buf = aligned_alloc(32, len);
    if (!src_buf) {
    ...
    // NOTE: These should be bus addresses, not virtual
    // In real implementation, would convert using /proc/self/maps
    cb->source_ad = (uint32_t)(uintptr_t)src_buf;
    ...
    dma_regs[1] = (uint32_t)(uintptr_t)cb;  // CONBLK_AD
...
int fuzz_iteration(const uint8_t* data, size_t size) {
    ...
    // Uncomment to actually perform SMC (DANGEROUS without proper setup):
